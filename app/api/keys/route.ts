...
import { createApiKey, getKeysByEmail, revokeKey, adminListAll, PLANS, storageBackend } from '@/lib/api-keys';
...
export async function POST(req: NextRequest) {
    try {
        ...
        // Admin puede crear cualquier plan; usuarios normales solo starter
        const resolvedPlan = adminSecret === ADMIN_PASS ? plan : 'starter';

        const { rawKey, record } = await createApiKey(email, resolvedPlan, label);
...
export async function GET(req: NextRequest) {
    ...
    if (adminSecret === ADMIN_PASS) {
        ...
        return NextResponse.json({ keys, backend: storageBackend() });
    }
